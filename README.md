...
- 학생은 Google 로그인 없이 학번·이름으로 신청, 담당자만 Google 로그인
- 신청자 정보는 학번·이름만 입력. 장비·목적·장소·일정은 대여 단계에서 입력
- 담당자 승인·반려·인도·반납·승인 취소, 최대 4종 / 30일 / 90일 이내 시작
...
학생은 학번과 이름만 입력하여 시작하며 내부적으로 Firebase 익명 인증 UID를 사용합니다. 학번·이름은 본인 확인 수단이 아니며 같은 정보를 입력한 다른 브라우저에 조회 권한을 주지 않습니다. 학생의 신청 내역은 해당 브라우저의 인증 저장소가 유지되는 동안 조회할 수 있습니다. 사용 종료, 브라우저 데이터 삭제, 다른 기기에서는 담당자에게 이전 내역을 문의해야 합니다. 익명 인증은 악의적인 다중 계정 생성을 막는 수단은 아닙니다. 기존 Google 사용자와 과거 신청 내역은 호환됩니다.
